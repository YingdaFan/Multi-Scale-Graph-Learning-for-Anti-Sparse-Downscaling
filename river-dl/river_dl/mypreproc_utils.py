...
def saveRunLog(config, code_dir, outFile):
    ...
    with open(outFile, "a+") as f:
        ...
        f.write("%s,'%s','%s'\n" % (datetime.date.today().strftime("%m/%d/%y"),
                                    os.path.join(os.path.relpath(os.getcwd(), code_dir), config['out_dir']),
                                    config['runDescription']))
...
def asRunConfig(config, code_dir, outFile):
    ...
    status = str(subprocess.Popen(['git status'], cwd=None if code_dir == "" else code_dir, shell=True,
    ...
    config['gitStatus'] = 'unknown' if len(status) == 1 else 'dirty' if len(modifiedFiles) > 0 or len(
        newFiles) > 0 else 'clean'
    # if the repo is dirty, make a zipped archive of the code directory
    if config['gitStatus'] == 'dirty':
        shutil.make_archive(os.path.join(os.path.dirname(outFile), "river_dl"), "zip",
                            os.path.join(code_dir, "river_dl"))
    ...
    config['input_file_info'] = {config[x]: {'file_size': os.stat(config[x]).st_size,
                                             'file_date': time.strftime("%m/%d/%Y %I:%M:%S %p",
                                                                        time.localtime(os.stat(config[x]).st_ctime))}
                                 for x in config.keys() if "file" in x and x != "input_file_info"}
...
def reduce_training_data_random(
    ...
    # 确定是否需要使用列名访问，适用于off这种列名flag，max_temp_c，mean_temp_c，mean_time，min_temp_c，network_ref的情况
    use_variable_name = len(ds.data_vars) > 1

    periods = [(train_start_date, train_end_date), (val_start_date, val_end_date)]
    reduced_indices = []

    for start_date, end_date in periods:
        if start_date is None or end_date is None:
            continue  # Skip if validation dates are not specified
        start_date = start_date if isinstance(start_date, list) else [start_date]
        end_date = end_date if isinstance(end_date, list) else [end_date]
        ...
        for s_date, e_date in zip(start_date, end_date):
            ...
            if date_level == 0:
                df_period = df.loc[idx[s_date:e_date, :], 'mean_temp_c'] if use_variable_name else df.loc[idx[s_date:e_date, :]]
            else:
                df_period = df.loc[idx[:, s_date:e_date], 'mean_temp_c'] if use_variable_name else df.loc[idx[:, s_date:e_date]]

            if segs is not None:
                if date_level == 0:
                    df_period = df_period.loc[idx[:, segs], :]
                else:
                    df_period = df_period.loc[idx[segs, :], :]
            non_null = df_period.dropna()

            reduce_idx = non_null.sample(frac=reduce_amount, random_state=random_seed).index
            reduced_indices.extend(reduce_idx)


    if use_variable_name:
        df.loc[reduced_indices, 'mean_temp_c'] = np.nan
    else:
            ...
            for t in gap_timesteps:
                ...
                for i in gap_ind:
                    ...
                    if gap_length < seq_len:
                        ...
                        print(
                            "The gap between this partition's continuous time periods is less than the sequence length")
...
def prep_all_data(
    ...
    #2023.10.10 加入seg作为字典值
    if not 'segs' in all_data.keys():
        all_data['segs'] = segs
