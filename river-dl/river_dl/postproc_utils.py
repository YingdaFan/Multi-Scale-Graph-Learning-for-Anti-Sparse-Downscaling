...
def trim_preds(obs, preds, spatial_idx_name="seg_id_nat", time_idx_name="date"):
    preds_trim = preds.reset_index()
    trim_obs = obs.reset_index()
    preds_trim = preds_trim[
        (preds_trim[time_idx_name] >= trim_obs[time_idx_name].min())
        & (preds_trim[time_idx_name] <= trim_obs[time_idx_name].max())
        & (preds_trim[spatial_idx_name].isin(trim_obs[spatial_idx_name].unique()))
        ]
    return preds_trim.set_index([time_idx_name, spatial_idx_name])
...
def fmt_preds_obs(pred_data,
    ...
    for var_name in pred_data.columns:
        ...
        preds_var = trim_preds(obs_var, preds_var, spatial_idx_name, time_idx_name)  # 2023.9.24
...
def plot_ts_obs_preds(pred_file, obs_file, start_data, end_data, index_start=0, index_end=3, spatial_idx_name="seg_id_nat", outfile=None):
    """
    ...
    """
    all_points = [] #2023.10 extract (x,y) for prediction
    combined = fmt_preds_obs(pred_data=pred_file, obs_file=obs_file, spatial_idx_name=spatial_idx_name)['mean_temp_c']
    # #2023.9.23 changed 'temp_c' into 'mean_temp_c'
    combined = combined.dropna(subset=['pred', 'obs'])
    counts = combined.groupby(combined.index.get_level_values(1)).count().sort_values("obs", ascending=False)
    combined = combined[combined.index.get_level_values(1).isin(counts.index[index_start:index_end])].reset_index(
        level=spatial_idx_name)
    # 将日期字符串转换为 datetime 对象
    start_date = pd.to_datetime(start_data)
    end_date = pd.to_datetime(end_data)

    # Filter data for dates within the specified range
    combined = combined[
        (combined.index.get_level_values(0) >= start_date) & (combined.index.get_level_values(0) <= end_date)]
    if spatial_idx_name == 'COMID':
        segs = np.unique(combined.COMID)
    elif spatial_idx_name == 'seg_id_nat':
        segs = np.unique(combined.seg_id_nat)
    else:
        print('no seg_id_nat or comid')
    num_plots = len(segs)
    fig, axes = plt.subplots(nrows=num_plots)
    for seg, ax in zip(segs, axes.flat):
        if spatial_idx_name == 'COMID':
            df = combined.loc[combined.COMID == seg]
        elif spatial_idx_name == 'seg_id_nat':
            df = combined.loc[combined.seg_id_nat == seg]
        else:
            print('no seg_it_nat or comid')
        ax.plot("pred", data=df, label="pred", alpha=0.3, color='red')
        if seg == 1641:  #2023
            points = list(zip(df.index, df['pred'])) #2023.10 extract (x,y) for prediction
            all_points.extend(points)


        ax.scatter(df.index, df["obs"], label='obs', alpha=0.3, s=1)
        ax.legend()
        ax.set_title(seg)
    plt.tight_layout()
    if outfile:
    ...
    if all_points:
        ...
        df_points.to_csv('D:/river/river-dl/workflow_examples/output_downscale_NHM/prediction.csv', index=False)
...
def prepped_array_to_df(data_array, dates, ids, col_names, spatial_idx_name='seg_id_nat', time_idx_name='date'):
    ...
    flat_data_combined = np.reshape(data_array, (-1, num_out_vars))
    flat_data_combined = flat_data_combined.numpy()
    dates = dates.flatten()
    ids = ids.flatten()
...
def combine_preds(fileList, weights=None, pred_vars=None, outFile="composite.feather", spatial_idx_name="seg_id_nat",
                  time_idx_name="date"):
    """
...
each model (range of 0 - 1). If None, the models are weighted equally
    ...
    np.testing.assert_allclose(weightCheckDF.modelWeight, 1, rtol=1e-02, atol=1e-02, equal_nan=True,
                               err_msg='Model weights did not sum to 1', verbose=True)
