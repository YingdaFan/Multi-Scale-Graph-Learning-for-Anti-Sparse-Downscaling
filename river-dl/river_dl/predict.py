...
from river_dl.preproc_utils import (scale,convert_batch_reshape,coord_as_reshaped_array)
...
if __name__ == '__main__':
    data = np.load("D:/river/river-dl/workflow_examples/output_test_pytorch/prepped.npz", )
    adj_mx = data['dist_matrix']
    in_dim = len(data['x_vars'])
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    model = RGCN_v1(in_dim, 20, adj_mx, device=device, seed=False)
    model.load_state_dict(torch.load("D:/river/river-dl/workflow_examples/output_test_pytorch/finetuned_weights.pth"))
    trn = predict_from_io_data(model=model,
                         io_data="D:/river/river-dl/workflow_examples/output_test_pytorch/prepped.npz",
                         partition='trn',
                         outfile="D:/river/river-dl/workflow_examples/output_test_pytorch/trn_preds.feather",
                         trn_offset=1.0,
                         tst_val_offset=1.0)
    val = predict_from_io_data(model=model,
                         io_data="D:/river/river-dl/workflow_examples/output_test_pytorch/prepped.npz",
                         partition='val',
                         outfile="D:/river/river-dl/workflow_examples/output_test_pytorch/val_preds.feather",
                         trn_offset=1.0,
                         tst_val_offset=1.0)
    tst = predict_from_io_data(model=model,
                         io_data="D:/river/river-dl/workflow_examples/output_test_pytorch/prepped.npz",
                         partition='tst',
                         outfile="D:/river/river-dl/workflow_examples/output_test_pytorch/tst_preds.feather",
                         trn_offset=1.0,
                         tst_val_offset=1.0)