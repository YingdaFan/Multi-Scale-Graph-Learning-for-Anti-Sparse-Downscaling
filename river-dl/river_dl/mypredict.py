...
def mypredict_from_io_data(
    ...
    preds = predict(
        ...
        io_data_x[f"ids_{partition}"], #2023.10.25 ids与y_prep_pp里面‘COMID‘/’seg_id_nat‘具体数值息息相关，我认为应该改成y的
        io_data_x[f"times_{partition}"], #2023.11.29 timedata如果采用x的，则为13*365行，与pred经过np.reshape之后的63*365对不上，我认为应该改成y的
        io_data_y["y_std"],
        io_data_y["y_mean"],
...
def predict(
    ...
    #y_pred_pp = prepped_array_to_df(y_pred, pred_dates, pred_ids, y_vars, y_spatial_idx_name, time_idx_name)
