...
def val_loop(dataloader, model, loss_function, device='cpu',y_std=None,y_mean=None):
    """
    @param dataloader: [object] torch dataloader with train and val data
    @param model: [object] initialized torch model
    @param loss_function: loss function
    ...
    for iter, (x, y) in enumerate(dataloader):
        ...
        y_std = torch.tensor(y_std).float().to(device) if isinstance(y_std, np.ndarray) else y_std.to(device).float()
        y_mean = torch.tensor(y_mean).float().to(device) if isinstance(y_mean, np.ndarray) else y_mean.to(
            device).float()
...
def train_torch(model,
    ...
    for i in range(max_epochs):
        ...
        '''
        if i==59: #2024.4.5 我把模型存贮函数改在了这里，跑完实验记得删除
        ...
        if x_tst is not None:
            model.eval()
            epoch_tst_loss = val_loop(tst_loader, model, loss_function, device,y_std,y_mean)
        ...
        if x_val is not None:
            s1 = time.time()
            model.eval()
            epoch_val_loss = val_loop(val_loader, model, loss_function, device, y_std, y_mean)
